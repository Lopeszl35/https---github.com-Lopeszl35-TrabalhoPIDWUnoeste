...
import { FaListAlt, FaPlus, FaSearch, FaEdit, FaTrashAlt, FaArrowUp, FaArrowDown } from "react-icons/fa";
...
function Materiais() {
    ...
    const handleSaida = (id) => {
        const novaLista = listaMateriais.map((item) => {
            if (item.id === id) {
                return { ...item, quantidade: item.quantidade === 0 ? 0 : parseInt(item.quantidade) -  1};
            }
            return item;
        });
        setListaMateriais(novaLista);
    ...
    return (
        <div className={`container-materiais ${show ? 'container-materiais-side-active' : ''}`}>
            <h1>
            ...
            <Container>
                <Card.Body className="mt-2 card-material">
                    <Row>
                        <Col lg='2'>
                            <Button as={Link} to='/materiais/cadastro' variant="primary"><FaPlus /> Adicionar</Button>
                        </Col>
                        <Col lg='6'>
                            <Form>
                        ...
                        <Col lg='2'>
                            <Button as={Link} to='/materiais/pesquisa/{id}' variant="secondary"><FaSearch /> Pesquisar</Button>
                        </Col>
                    </Row>
                </Card.Body>
            </Container>
            <Container>
                ...
                <Table striped bordered hover className='table'>
                    ...
                    <tbody>
                        {listaMateriais.length <= 0 ? (<tr><td colSpan={7}>Nenhum material encontrado</td></tr>) :
                            listaMateriais.map(material => (
                                <>
                                    <tr key={material.id} onClick={() => toggleEntradaSaida(material.id)} style={{ cursor: 'pointer' }}>
                                        <td>{material.id}</td>
                                        <td>{material.nome}</td>
                                        ...
                                        <td className='d-flex flex-row'>
                                            <Link to={`/materiais/editar/${material.id}`} className="btn btn-primary m-1 w-100" onClick={(e) => e.stopPropagation()}> <FaEdit></FaEdit>  Editar</Link>
                                            <Button className="btn btn-danger m-1 w-100" onClick={(e) => { e.stopPropagation(); abrirModalConfirmacao(material.id); }}> <FaTrashAlt></FaTrashAlt>  Excluir</Button>
                                        </td>
                                    </tr>
                                    {entradaSaida === material.id && (
                                        <tr key={`entradaSaida-${material.id}`}>
                                            <td colSpan={7} className='p-2'>
                                                <Button variant="success" className='m-1' onClick={(e) => { e.stopPropagation(); handleEntrada(material.id); }}>
                                                    <FaArrowUp></FaArrowUp> Registrar Entrada
                                                </Button>
                                                <Button variant="danger" className='m-1' onClick={(e) => { e.stopPropagation(); handleSaida(material.id); }}>
            ...
            <Modal show={showConfirmDeleteModal} onHide={fecharModalConfirmacao}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="danger" onClick={() => handleExcluir(materialToDelete)}>
                        Excluir
                    </Button>
                </Modal.Footer>
            </Modal>
